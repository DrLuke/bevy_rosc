...
fn send_message(mut disp: ResMut<OscDispatcher>, time: Res<Time>, mut query: Query<(&mut OscMethod)>) {
    let mut new_msg = OscMessage { addr: "/entity*/time".to_string(), args: vec![time.time_since_startup().as_secs_f32().into()] };
